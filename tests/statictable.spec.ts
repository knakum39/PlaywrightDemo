...
/* for (let row of ...) → selects which row
locator('td') → selects columns/cells inside that row
nth(0) → first column
nth(1) → second column

A simple memory trick:

Loop chooses the row. nth chooses the column. ✅ */
...
for(let row of allRowData.slice(1))   // slice(1) --> skip header row 
{
    ...
    //const price=cells[3]; // capture price from 3th index , Here price is in string format.
    ...
    totalPrice=totalPrice+parseInt(price);//ParseInt is a method used to conver string to integer.
